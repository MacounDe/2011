	...
	if (! running) {
		if (! initialized) {
			...
			startButton.hidden = NO;
                ...
                [serverConnection sendWaypoint:[Waypoint waypointWithLocation:newLocation]];
...
- (IBAction) startRun:(id)sender {
	...
	AudioServicesPlaySystemSound(goSound);
        [serverConnection sendWaypoint:[Waypoint waypointWithLocation:lastLocation]];
}
...
- (void) initServerConnection {
    if (! serverConnection) 
        serverConnection = [[ServerConnection alloc] initWithDelegate:self];
    
    NSString * username = [[NSUserDefaults standardUserDefaults] objectForKey:MRUsername] ;
    NSString * passwordHash = [[NSUserDefaults standardUserDefaults] objectForKey:MRPasswordHash];
    
    if (! [username length] || ! [passwordHash length]) {
        [self showLoginAlert];
    } else {
        [serverConnection loginWithUsername:username passwordHash:passwordHash];
    }
}
- (void) connectionFailed:(NSString*) reason {
    NSLog(@"Fehler: %@",reason);
        [self performSelectorOnMainThread:@selector(showLoginAlert) withObject:nil waitUntilDone:NO];
}

- (void) showLoginAlert {
	LoginAlertView *loginAlert = [[LoginAlertView alloc] initWithAlertViewDelegate:self];
	[loginAlert show];
    [loginAlert release];
}
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != [alertView cancelButtonIndex]) {
        NSString * newUsername = ((LoginAlertView *)alertView).nameField.text;
        NSString * password = ((LoginAlertView *)alertView).passwordField.text;
        
        [[NSUserDefaults standardUserDefaults] setObject:newUsername forKey:MRUsername];
        [[NSUserDefaults standardUserDefaults] setObject:[self sha1:password] forKey:MRPasswordHash];
