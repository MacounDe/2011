...
NSString* HIDPluggedNotification = @"HID plugged";
NSString* HIDUnpluggedNotification = @"HID unplugged";
...
- (void) devicePlugged:(IOHIDDeviceRef)ref {
	...
	if (device) {
		...
		[nc postNotificationName:HIDPluggedNotification
						  object:device
						userInfo:nil];
...
- (void) deviceUnplugged:(IOHIDDeviceRef)ref {
	...
	for (HIDDevice* device in self.pluggedDevices) {
		if (device.deviceRef != ref) [remaining addObject:device];
			...
			NSNotificationCenter* nc = [NSNotificationCenter defaultCenter];
			[nc postNotificationName:HIDUnpluggedNotification
							  object:device
							userInfo:nil];
			NSLog(@"device unplugged: %@",device);
		}
	}	
